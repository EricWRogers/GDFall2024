...
public class SquareKey : MonoBehaviour
{
    public bool hasSquareKey = false;  // Tracks whether the player has picked up the Square Key
    ...
    private void Start()
    {
        ...
        if (squareKeyImage != null)
        {
            squareKeyImage.enabled = false;  // Hide the square key image initially
        }
    ...
    // Pick up the Square Key when the player presses 'E'
    void PickUpSquareKey()
    {
        // Set the hasSquareKey flag to true
        hasSquareKey = true;

        // Update the message to show key picked up
        if (pickupMessageText != null)
        {
            pickupMessageText.text = "Square Key picked up!";
        }

        // Show the key image in the UI
        if (squareKeyImage != null)
        {
            squareKeyImage.enabled = true;  // Make the square key image visible
        }
        ...
        Debug.Log("Square Key picked up!");
    }
}