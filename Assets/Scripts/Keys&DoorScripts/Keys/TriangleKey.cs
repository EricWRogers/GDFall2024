...
{
    public bool hasTriangleKey = false;  // Tracks whether the player has picked up the Triangle Key
    ...
    private void Start()
    {
        ...
        if (triangleKeyImage != null)
        {
            triangleKeyImage.enabled = false;  // Hide the triangle key image initially
        }
    ...
    // Pick up the Triangle Key when the player presses 'E'
    void PickUpTriangleKey()
    {
        // Set the hasTriangleKey flag to true
        hasTriangleKey = true;

        // Update the message to show key picked up
        if (pickupMessageText != null)
        {
            pickupMessageText.text = "Triangle Key picked up!";
        }

        // Show the key image in the UI
        if (triangleKeyImage != null)
        {
            triangleKeyImage.enabled = true;  // Make the triangle key image visible
        }
        ...
        Debug.Log("Triangle Key picked up!");
    }
}